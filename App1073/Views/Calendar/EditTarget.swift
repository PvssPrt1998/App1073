...
struct EditTarget: View {
    ...
    init(show: Binding<Bool>, target: Target) {
        self._show = show
        self.startDate = target.startDate
        self.name = target.name
        self.endDate = target.endDate
        self.description = target.description
        self.selectedLabel = target.label
        self.selectedStatus = target.status
    }
    ...
    var body: some View {
        NavigationView {
            ...
            .onAppear {
                self.selectedLabel = source.targetForEdit?.label
                self.selectedStatus = source.targetForEdit?.status
            }
        }
    }
            ...
            Text("Calendar")
                ...
                .overlay(
                    HStack {
                        ...
                        Spacer()
                        Button {
                            source.editTarget(name: name, startDate: startDate, endDate: endDate, description: description, status: selectedStatus!, label: selectedLabel!)
                            show = false
                        } label: {
                            Text("Done")
                                .font(.system(size: 17, weight: .regular))
...
struct EditTarget_Preview: PreviewProvider {
    
    @State static var show: Bool = true
    
    static var previews: some View {
        EditTarget(show: $show, target: Target(uuid: UUID(), name: "123", startDate: "05.05.1990", endDate: "12.12.1212", description: "descr", status: Status(id: 0, name: "To do", color: .red), label: Label(id: 0, name: "School", color: .red)))
